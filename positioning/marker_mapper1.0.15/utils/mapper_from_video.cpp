    ...
    try{
     ...
     //   aruco_mm::debug::Debug::setLevel(5);
        ...
        for(auto vfile:videos){
            ...
            //        while(frame<6924){vcap.grab();frame++;}
             ...
             do{
                ...
                // 左右切分
                cv::Mat left_half = image(cv::Rect(0, 0, width / 2, height));
                cv::Mat right_half = image(cv::Rect(width / 2, 0, width / 2, height));
                ...
                image = left_half;
                left_half.copyTo(image2);

                frame++;
                if( frame%frameIncrement!=0) continue;
                AMM-> process ( image ,frame);
                AMM-> drawDetectedMarkers ( image2);
                cv::resize(image2,img_resized,cv::Size(min( image2.cols,3840),min(image2.rows,2160)));
        ...
        if (!cml["-noshow"]){
            ...
            Viewer.setParams(mmap,1.5,960,540,"map_viewer");
            key=0;
            while(key!=27)
                key =     Viewer.show( );
        }
